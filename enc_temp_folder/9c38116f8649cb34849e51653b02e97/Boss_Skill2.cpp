...
void UBoss_Skill2::SpawnMonsters()
{
    ...
    if (!World)
    {
        UE_LOG(LogTemp, Error, TEXT("BossRef->GetWorld() returned NULL in SpawnMonsters"));
        return;
    }

    // BossRef에서 설정한 Skill2SpawnCount만큼 소환
    for (int32 i = 0; i < BossRef->Skill2SpawnCount; i++)
    {
        // Skill2MonsterList에서 무작위 몬스터 블루프린트를 선택
        ...
        if (!MonsterClass)
        {
            UE_LOG(LogTemp, Warning, TEXT("MonsterClass is NULL at index %d"), RandomIndex);
            continue;
        }

        float Distance = FMath::RandRange(100.0f, BossRef->Skill2SpawnRadius);
        ...
        if (SpawnedMonster)
        {
            UE_LOG(LogTemp, Log, TEXT("Spawned monster at %s"), *SpawnLocation.ToString());
        }
        else
        {
            UE_LOG(LogTemp, Warning, TEXT("Failed to spawn monster of class: %s"), *MonsterClass->GetName());
        }
    }
}
