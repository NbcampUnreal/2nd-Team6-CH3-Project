...
void UBTTask_BossAttack3::Attack3_RangedAttackNotify()
{
    ...
    // 보스의 회전값에 ±30도 범위 내에서 랜덤한 Yaw 오프셋을 추가
    float RandomYawOffset = FMath::RandRange(-30.0f, 30.0f);
    FRotator BulletRotation = CurrentRotation;
    BulletRotation.Yaw += RandomYawOffset;  // Yaw 값에 오프셋을 더함

    // 총알의 방향을 계산 (회전값을 기준으로)
    FVector BulletDirection = BulletRotation.Vector();  // 회전값으로 방향 계산

    // 총알 발사 위치는 보스의 무즈 위치로 설정
    ...
    if (Bullet)
    {
        Bullet->FireProjectile(SpawnLocation, BulletRotation, BulletDirection);  // 위치, 회전, 방향
    }
